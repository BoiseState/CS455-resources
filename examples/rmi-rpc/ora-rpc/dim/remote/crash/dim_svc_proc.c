...
#define FIX_RETURN(fp, seekPt, s) { \
        fclose(fp); \
        truncate(SERVERDB, (off_t) seekPt); \
        sprintf(s, "SERVERDB %s data hosed at %ld, repaired\n", \
          SERVERDB, seekPt); \
        fprintf(stderr, s); \
        return; \
      }

repairDB(s)				/* does something now */
  pStr            s;
{
  int             fstat, b, x, y, d, c;
  long            seekPt, fo;
  ...
  /*
	* Don't know exactly where the file pointer is at this time,
	* so rewind and stop at a point where the database gets funky.
	* If we cannot read a complete entry we assume it's a corrupted add
	* or delete operation.  A BIG assumption just for purpose of
	* illustration.
	*/
  fclose(fp);
  if (!(fp = fopen(SERVERDB, "r"))) {
    sprintf(s, "cannot open SERVERDB %s for reading\n", SERVERDB);
    fprintf(stderr, s);
    return;
  }

  for (seekPt = 0;
	 (fstat = fscanf(fp, "%d%d%d%d%d\n", &b, &x, &y, &d, &c)) != EOF;
	 seekPt = ftell(fp) ) {
    switch (fstat) {
      case 5:			/* everything is fine, try to get to next one */
        READHEADER(N, O, C, D);
		  if (feof(fp)) FIX_RETURN(fp, seekPt, s);
		  fo = ftell(fp);
        fseek(fp, (long) b-1, 1);
		  dummy = fgetc(fp);
		  if (feof(fp) || ((ftell(fp) - fo) != b)) FIX_RETURN(fp, seekPt, s);
		  break; /* all went well */
      default:			/* didn't get all 5 values, blast it */
		  FIX_RETURN(fp, seekPt, s);
    }
  }
}
