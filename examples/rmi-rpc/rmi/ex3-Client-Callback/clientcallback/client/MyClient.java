...
import clientcallback.server.Server;
import clientcallback.server.StringEnumeration;
import clientcallback.server.WorkListener;
...
public class MyClient extends java.rmi.server.UnicastRemoteObject implements WorkListener {
	...
	public MyClient(String host) throws RemoteException 
	{
		try {
			...
			/*
			Note that since client is also a RMI server (for callbacks from the
			server), it doesn't terminate. That is why we use System.exit to
			force termination of the client. This use isn't correct--find a way
			to fix it.
			*/
			System.exit(0);
