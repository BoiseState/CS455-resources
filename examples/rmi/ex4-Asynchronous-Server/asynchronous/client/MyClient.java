...
/**
 * Shows a client making multiple asynchronous calls to a server. 
 * However the client now needs to keep track of the number of outstanding asynchronous
...
public class MyClient extends java.rmi.server.UnicastRemoteObject implements WorkListener 
{
	...
	public MyClient(String host) throws RemoteException
	{
		...
		try {
			...
			incCount(); server.asyncExecute( new MyCalculation(10000), this ); 
			incCount(); server.asyncExecute( new MyCalculation(1000), this );
			incCount(); server.asyncExecute( new MyCalculation(10), this ); 
			incCount(); server.asyncExecute( new MyCalculation(50), this ); 
			requestsSent = true;

			try {synchronized (allDone) { allDone.wait(); } }
	...
	public synchronized void decCount() {count--;}
